...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Card className="group relative flex h-full flex-col overflow-hidden rounded-none border-0 bg-transparent p-0 shadow-none transition-all duration-300 dark:bg-transparent">
      ...
      <CardContent className="flex grow flex-col justify-between gap-2 p-5">
        <Link href={`/products/${product.slug}`} className="block">
          <h3 className="line-clamp-2 text-sm font-medium leading-tight text-zinc-900 transition-colors group-hover:text-zinc-600 dark:text-zinc-100 dark:group-hover:text-zinc-300">
            {product.name}
          </h3>
        </Link>
