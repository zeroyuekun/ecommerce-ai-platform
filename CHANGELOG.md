...
- **Pending:** T10 smoke run (real agent + faithfulness numbers) + T18 final appendix — still blocked on Vercel AI Gateway free-tier abuse limit. The first eval trace fires fine (verified 2026-04-28); the agent's Sonnet call returns `GatewayRateLimitError 429`. Resolution: top up credits at vercel.com/[team]/~/ai?modal=top-up, then `pnpm eval:rag --yes`. The no-LLM baseline above already proves the harness wiring + retrieval quality on the full 50-case set.
