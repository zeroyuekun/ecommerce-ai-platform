...
0. **Google free-tier daily quotas don't reset on a clean local-midnight tick.** Empirically (2026-04-28 → 2026-04-29) the bucket carries forward into the next local day for several hours after exhaustion. Practical implication: a daily cron pacing batches across days is *unreliable* on a single project — the schedule can't assume each firing finds a fresh budget. The Path-2 day-by-day strategy in the four paths above looked clean on paper and isn't.
