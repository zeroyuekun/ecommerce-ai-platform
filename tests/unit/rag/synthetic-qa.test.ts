...
import { generateSyntheticQa, type QaGenerator } from "@/lib/ai/rag/indexer/synthetic-qa";
...
describe("generateSyntheticQa", () => {
  it("returns the generator's output unchanged when valid", async () => {
    ...
    expect(gen).toHaveBeenCalledWith(expect.objectContaining({ product: baseProduct, count: 5 }));
  });
